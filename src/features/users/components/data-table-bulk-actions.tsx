...
export function DataTableBulkActions<TData>({
    ...
    try {
      ...
      await Promise.all(
        selectedUsers.map((user) =>
          updateUser(user.id, { status: status as User['status'] })
        )
      )
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : `Error ${status === 'active' ? 'activating' : 'deactivating'} users`
      )
    }
  }
  ...
  const handleBulkInvite = async () => {
    try {
      ...
      // Update all selected users to 'invited' status
      await Promise.all(
        selectedUsers.map((user) =>
      ...
      toast.error(
        error instanceof Error ? error.message : 'Error inviting users'
      )
    }
  }
